SELECT target_gene_source_value+'_'+hgvs_p as name,
		ROUND(COUNT(distinct(person_id))/CAST((SELECT COUNT(person_id)
                                          FROM [@schema].[dbo].[condition_occurrence]
                                          WHERE person_id IN (SELECT [person_id]
                                          FROM [@schema].[dbo].[specimen]
                                          WHERE [anatomic_site_source_value] = 'Lung')
                                          AND [condition_source_value] IN ('Invasive adenocarcinoma', 'Minimally invasive adenocarcinoma', 'Squamous cell carcinoma'))AS float),4)*100 as fraction
